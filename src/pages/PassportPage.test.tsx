...
describe("passport", () => {
  ...
  it("starts with an anonymous empty passport", () => {
    ...
    expect(screen.getByText("0 of 6 stamps")).toBeInTheDocument();
  ...
  it("collects a stamp from a valid QR route", () => {
    renderApp("/passport/collect/mece-design-01");
    ...
    expect(storedPassport).toContain("stamp-design");
  });

  it("does not duplicate an already collected stamp", () => {
    renderApp("/passport/collect/mece-design-01").unmount();
    renderApp("/passport/collect/mece-design-01");
  ...
  it("restores collected progress on a later visit", () => {
    ...
    expect(screen.getByText("1 of 6 stamps")).toBeInTheDocument();
    expect(screen.getByText("Design Explorer")).toBeInTheDocument();
  });
});
